...
import { ShieldCheck, Loader2, RotateCcw, HelpCircle, ArrowLeft, FileText } from 'lucide-react';
...
import { SAMPLE_FILE_CONTENT } from './constants';
...
const App: React.FC = () => {
  ...
  const handleSampleAnalysis = async () => {
    // Convert sample text to base64 safely
    const base64Data = window.btoa(unescape(encodeURIComponent(SAMPLE_FILE_CONTENT)));
    
    const sampleFile: ProcessedFile = {
        name: "Audit_Dataset_Zephyr_Supply_Chain.txt",
        type: "text/plain",
        mimeType: "text/plain",
        data: base64Data
    };
    ...
    try {
        const result = await analyzeDocuments([sampleFile]);
        setReport(result);
        setStatus(AppStatus.COMPLETE);
    } catch (err: any) {
        console.error("Sample analysis failed", err);
        setErrorMsg(err.message || "Failed to analyze sample data.");
        setStatus(AppStatus.ERROR);
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans">
      ...
      <header className="sticky top-6 z-50 px-4 md:px-8 animate-slide-down">
      ...
      <main className="flex-grow pt-12 pb-20 px-4">
        <div className="max-w-7xl mx-auto">
          ...
          {status !== AppStatus.COMPLETE && (
            <div className={`max-w-4xl mx-auto transition-all duration-700 ease-[cubic-bezier(0.34,1.56,0.64,1)] ${status === AppStatus.ANALYZING ? 'scale-95 opacity-50 blur-[2px] pointer-events-none' : 'scale-100 opacity-100 animate-slide-up'}`}>
              <div className="glass-panel rounded-[2.5rem] p-8 md:p-12 mb-8 relative overflow-hidden">
                 ...
                 <div className="mt-10 flex flex-col items-center justify-center gap-4 relative z-10">
                   <button
                     ...
                     className={`
                       relative overflow-hidden group flex items-center gap-3 px-10 py-5 rounded-2xl font-bold text-lg transition-all duration-300 transform w-full md:w-auto justify-center
                       ${files.length === 0 
                   ...
                   {files.length === 0 && (
                       <button 
                           ...
                           className="text-sm font-semibold text-slate-500 hover:text-neon-blue-electric transition-colors flex items-center gap-2 px-4 py-2 rounded-lg hover:bg-slate-100/50"
                       >
                           <FileText size={14} />
                           <span>Use Sample Audit Dataset</span>
                       </button>
                   )}
                 </div>
              </div>
          ...
          {status === AppStatus.ERROR && (
             <div className="max-w-3xl mx-auto mb-8 bg-red-50/80 backdrop-blur-xl border border-red-200/50 rounded-3xl p-6 flex items-center gap-4 text-red-700 shadow-lg animate-slide-up">
